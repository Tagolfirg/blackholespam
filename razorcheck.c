/* razorcheck.c */
static char *id = 
     "$Id: razorcheck.c,v 1.12 2002/08/16 16:22:35 bitbytebit Exp $";
/*
   Copyright (C) 2002
...
extern char *razor_bin;
extern char *perl_bin;
...
int razor_check(void)
{
  ...
  if(pid == 0) {
    ...
    execv(perl_bin, razor_args);

    /* Error if still here */
    fprintf(stderr, "Error Razor Checking\n");
    exit(127);
  }
  do {
    if(waitpid(pid, &status, 0) == -1) {
      if(errno != EINTR) {
        fprintf(stderr, "%s: Error Executing Razor! %s:%d\n",
                __FILE__, __FILE__, __LINE__);
        return -1;
      }
    } else {
      ...
      if(status == 0) {
        ...
        if(DEBUG)
          fprintf(stderr, " Match Razor %s\n", log_info);

        return 1;
      } else if(DEBUG && status != 1)
        fprintf(stderr, "Error with Razor (%d)\n", status);
