/* checkctype.c */
static char *id = 
     "$Id: checkctype.c,v 1.8 2002/08/16 16:22:35 bitbytebit Exp $";
/*
   Copyright (C) 2002
...
int check_ctype(char *line, char *badctype[])
{
  int i, j;
  char *p = NULL;
  char *buffer;
  ...
  /* Check charsets */
  for(i = 0; badctype[i] != NULL; i++) {
    if(strstr(buffer, badctype[i]) != NULL) {
      strsize = my_strlen(buffer) + my_strlen(badctype[i]) + 3;
      log_info = malloc(strsize + 1);
      if(log_info == NULL)
        return (0);

      snprintf(log_info, strsize + 1, "(%s) %s", buffer, badctype[i]);
      log_info[strsize] = (char)'\0';

      if(DEBUG)
        fprintf(stderr, " Match Bad Content Type %s\n", log_info);
